...
* ReentrantLock是个类（注意Lock是接口），synchronized是关键字，当然都是在JVM层面实现互斥锁的方式（原始CAS是CPU层面）
...
多个线程竟争锁资源,如果线程A通过CAS方式先执行了,则代表线程A拿到了锁资源,state变成1,head,tail不变
