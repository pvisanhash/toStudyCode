...
* ReentrantLock是个类（Lock是接口），synchronized是关键字，当然都是在JVM层面实现互斥锁的方式（原始CAS是CPU层面）
...
ReentrantLock的功能比synchronized更全面：
  * ReentrantLock支持公平锁和非公平锁，但synchronized只支持非公平锁
...
AQS就是AbstractQueuedSynchronizer类（抽象的对列同步器），AQS其实就是**JUC包下的一个基类**，JUC下的很多内容都是基于AQS实现了部分功能，比如ReentrantLock，ThreadPoolExecutor，阻塞队列，CountDownLatch，Semaphore，CyclicBarrier等等都是基于AQS实现。
...
新入队列的结点的waitStatus=0  并将上一结点的waitStatus 改为 -1
