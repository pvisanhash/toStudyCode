...
>线程优先级:
>
>- **从Java语言规范层面看**：`setPriority` 的值越大，线程优先级越高。
>- **从JVM实现层面看**：JVM会尽力尊重这个设置。
>- **从实际系统运行层面看**：**最终决定权在操作系统调度器手中**，优先级的影响可能是微乎其微甚至不可预测的。
>
>因此，作为高级工程师，我们的结论是：**了解它，但不要构建强依赖于它的程序逻辑。**



##### 3.2.4 线程的yield()让步

可以通过Thread的静态方法yield，让当前线程从运行状态转变为就绪状态(它是一个“建议”，并且行为是不确定的，不建议使用)。
...
答：如果从Java定义的状态来看就是Runnable -> Runnable,如果是从操作系统看是从Running到Ready状态.

##### 3.2.5 线程的sleep(long millis)休眠

Thread的sleep静态方法，让线程从运行状态转变为时间等待状态。
...
思考：如果当前线程调用Thread.sleep(long millis)方法后，状态由什么状态转为什么状态？
...
默认情况下，线程都是 **非守护线程(前台线程)**

JVM会在程序中没有**前台线程**时，就会结束掉当前JVM

主线程默认是**非守护线程**。如果主线程执行结束，需要查看当前JVM内是否还有非守护线程，如果没有非守护线程JVM就会直接停止。
...
- wait(long timeout, int nanos) // 限时等待
...
![](https://raw.githubusercontent.com/pvisanhash/PicSiteRepo1/main/note/img/202306100231291.png)
...
**CAS的缺点**：CAS只能保证对一个变量的操作是原子性的，无法实现对多行代码实现原子性,synchrnized可以保证。
...
Lock锁是在JDK1.5由Doug Lea研发的，他的性能相比synchronized在JDK1.5的时期，性能好了很多，但是在JDK1.6对synchronized优化之后，性能相差不大，但是如果涉及并发比较多时，推荐ReentrantLock锁(Lock锁的一种)，性能会更好。
...
Java中的引用类型分别是**强Strong，软Soft，弱Weak，虚Phantom**。

**强引用 Strong Reference**：在 Java 中最常见的就是强引用，把一个对象赋给一个引用变量，这个引用变量就是一个强引用。当一个对象被强引用变量引用时，它始终处于可达状态，它不可能被垃圾回收机制回收的，即使该对象以后永远都不会被用到, JVM 也不会回收。因此强引用是造成 Java 内存泄漏的主要原因之一。
...
* 每个Thread中都存储着一个成员变量：ThreadLocalMap，ThreadLocal本身不存储数据，像是一个工具类，基于ThreadLocal去操作ThreadLocalMap

![](https://raw.githubusercontent.com/pvisanhash/PicSiteRepo1/main/note/img/202304280008554.png)

* ThreadLocalMap本身就是基于Entry[]实现的，因为一个线程可以绑定(new)多个ThreadLocal，这样一来，可能需要存储多个数据，所以采用Entry[]的形式实现，Entry的key是ThreadLocal对象，value是要保存的值。
...
这就带来了问题，现在CPU都是多核，每个线程的工作内存（CPU三级缓存）都是独立的，会告知每个线程做修改时，只改自己的工作内存，如果没有及时的同步到主内存，导致数据不一致问题。

MESI = Modified / Exclusive / Shared / Invalid
这个CPU级别的协议就是解决多线程间的可见性问题，但各厂商适配的不一致，所以Java在CPU与内存间加了JMM内存模型。

![image.png](https://raw.githubusercontent.com/pvisanhash/PicSiteRepo1/main/note/img/202304261305384.png)
...
可见，Lock锁通过修改volatile修饰state属性从而将其他属性也变得可见了（CPU缓存失效，读写操作直接与主内存相关）。

看下lock.lock()方法

![](https://raw.githubusercontent.com/pvisanhash/PicSiteRepo1/main/note/img/202304301128200.png)

所以，我们可以举一反三得到，如果我们给任意变量加上volatile修饰 并放到代码块中，也能让实现所有相关变量的可见性。
...
2) 为什么懒汉式单例模式synchronized要放在if代码块内?
	防止每次进入方法时获取锁操作浪费资源

3) 为什么懒汉式单例模式需要double check?
	防止已经new过对象后,进入位置b的线程获取锁后再次new对象
...
**锁膨胀**：如果在一个**循环**中，频繁的获取和释放锁资源，这样带来的消耗很大，锁膨胀就是将锁的范围扩大，避免频繁的竞争和获取锁资源带来不必要的消耗。
...
![](https://raw.githubusercontent.com/pvisanhash/PicSiteRepo1/main/note/img/202305100038488.png)
...
![](https://raw.githubusercontent.com/pvisanhash/PicSiteRepo1/main/note/img/202305100044163.png)
...
适当的查看`objectMonitor.cpp`几个C++中实现的加锁流程
...
其次AQS中维护了一个双向链表，有head，有tail，并且每个节点都是Node对象（这就是队列的意思）
...
（图中的4.2 是将B线程的Node挂到tail的后面）

如果此时又来个线程C通过CAS方式没能拿到锁,也是将自己封装成Node对象,并挂在tail后面,将前一个节点的waitStatus值改为-1.
...
// 公平锁实现
   ...
   public final boolean hasQueuedPredecessors() {
       ...
       return h != t &&
           ...
           // 或者：队列正在初始化，第一个节点还没链好 或者：第一个节点不是当前线程
           // s == null 可以看addWaiter()方法理解
        ...
        if (node == tail && compareAndSetTail(node, pred)) {
            ...
            if (pred != head &&
                ...
                if (next != null && next.waitStatus <= 0)
	                // `next.prev = pred`并不是在`compareAndSetNext(pred, predNext, next)`
	                // 中设置的，而是在后继节点自身执行`shouldParkAfterFailedAcquire`
	                // 方法时完成的。
...
> 释放锁参考B站视频：【AQS源码分析(四)[lock流程图总结与unlock源码分析]】 https://www.bilibili.com/video/BV1mP4y1i7Vm/?share_source=copy_web&vd_source=9cd08437a297637267598afff1968894
...
像synchronized提供了wait和notify的方法实现线程在持有锁时，可以实现挂起与唤醒的操作。
...
![](https://raw.githubusercontent.com/pvisanhash/PicSiteRepo1/main/note/img/202307091117741.png)
...
虽然Node对象有prev和next，但是在ConditionObject中是不会使用这两个属性的，只要在Condition队列中，这两个属性都是null。在ConditionObject中只会使用nextWaiter的属性实现单向链表的效果。如果发现Node的prev或next不是null,代表当前节点不在ConditionObject队列中,而在AQS的队列中.
...
* 没有中断，就将当前线程封装为Node对象添加到Condition的单向链表中
* 一次性释放掉锁资源。
...
public final void await() throws InterruptedException {
    ...
    if (Thread.interrupted())
        ...
        throw new InterruptedException();
    // 只有持有锁的方法,才能执行await()方法,所以AQS队列一定没有此线程
...
// 线程挂起前，添加到Condition单向链表的业务~~
private Node addConditionWaiter() {
    // 拿到尾节点。
    Node t = lastWaiter;
    // 前面已经清楚,Node状态默认为0,1是取消状态,-1是代表后继节点有挂起的线程
    // 这里-2代表的就是condition等待的情况
...
private void doSignal(Node first) {
    ...
    do {
        ...
        if ( (firstWaiter = first.nextWaiter) == null)
            // 说明原来就一个节点在Condition队列中，那么直接将firstWaiter和lastWaiter置位null
            lastWaiter = null;
...
final boolean transferForSignal(Node node) {
    ...
    if (!compareAndSetWaitStatus(node, Node.CONDITION, 0))
        ...
        return false;
    // 如果正常的将Node的状态从-2改为0，这时就要将Condition中的这个Node扔到AQS的队列。
...
private int checkInterruptWhileWaiting(Node node) {
    ...
    return Thread.interrupted() ?
        ...
        // 虽然这时候不需要抛异常,但由于前面判断中断标时已清除中断标志位信息,这时候需要标记为再中断
...
final boolean transferAfterCancelledWait(Node node) {
    // 基于CAS将Node的状态从-2改为0
    // 前面signal()方法已将节点状态从-2改成0
    if (compareAndSetWaitStatus(node, Node.CONDITION, 0)) {
        // 这里成功,就说明是中断唤醒的线程,而不是singal()方法唤醒。因为CAS成功了。
...
![image.png](https://raw.githubusercontent.com/pvisanhash/PicSiteRepo1/main/note/img/202304261305395.png)
...
![image.png](https://raw.githubusercontent.com/pvisanhash/PicSiteRepo1/main/note/img/202304261305396.png)
...
>
> 基于cachedHoldCounter缓存最后一个拿到锁资源的重入次数
>
> ==============================最后一个获取读锁资源的重入次数记录方式================
>
> 重入次数的流程执行方式：
    ...
    if (!readerShouldBlock() &&
        ...
        // 到这，就说明不是第一个获取读锁资源的线程
            ...
            // 当前线程是之前的cacheHoldCounter
                ...
                // 将当前的重入信息设置到ThreadLocal中,因为rh.count==0代表是最新值所以要重设下
                readHolds.set(rh);
            // 重入的++
            rh.count++;
        }
...
注意: readHolds是在Sync构造器中new出来的

![image.png](https://raw.githubusercontent.com/pvisanhash/PicSiteRepo1/main/note/img/20251125001515991.png)
...
![image.png](https://raw.githubusercontent.com/pvisanhash/PicSiteRepo1/main/note/img/202304261305397.png)
...
![image.png](https://raw.githubusercontent.com/pvisanhash/PicSiteRepo1/main/note/img/202304261305398.png)
...
![image.png](https://raw.githubusercontent.com/pvisanhash/PicSiteRepo1/main/note/img/202304261305399.png)
...
// 常是临时辅助变量，用于构造/快照/批量操作,PBQ的底层是用queue对象,而不是这个q对象
private PriorityQueue<E> q;
```

#### 4.4 PriorityBlockingQueue的写入操作

毕竟是阻塞队列，添加数据的操作，咱们是很了解，无法还是add，offer，offer(time,unit)，put。但是因为优先级队列中，数组是可以扩容的，虽然有长度限制，但是依然属于无界队列的概念，所以生产者不会阻塞，所以只用查看下offer方法。
...
读取操作是存在挂起的情况的，因为如果数组中元素个数为0，当前线程如果执行了take方法，必然需要挂起。

获取数据时，因为是优先级队列，所以需要从二叉堆栈顶拿数据，直接拿索引为0的数据即可，但是拿完之后，需要保持二叉堆结构，所以会有下移操作。
...
一定要以局部的方式去查看树结构的变化，他是从根节点往下找较小的一个子节点，将较小的子节点挪动到父节点位置，再将循环往下走，如果一来，整个二叉堆的结构就可以保证了。
...
DelayQueue也是基于二叉堆结构实现的，甚至本身就是基于PriorityQueue实现的功能。二叉堆结构每次获取的是栈顶的数据，需要让DelayQueue中的数据，在比较时，跟根据延迟时间做比较，剩余时间最短的要放在栈顶。
...
可以每下一个订单，就放到延迟队列中，如果规定时间内，商家没有接单同，直接通过消费者获取元素，然后取消订单。
...
// leader一般会存储等待栈顶数据的消费者线程，在整体写入和消费的过程中，会设置的leader的一些判断。
...
![image.png](https://raw.githubusercontent.com/pvisanhash/PicSiteRepo1/main/note/img/202304261305400.png)

![image.png](https://raw.githubusercontent.com/pvisanhash/PicSiteRepo1/main/note/img/202304261305401.png)
...
![img.png](https://raw.githubusercontent.com/pvisanhash/PicSiteRepo1/main/note/img/20251211010517764.jpg)
