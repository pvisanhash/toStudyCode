...
public class RobotWalk {
    ...
    private static int process1(int cur, int rest, int N, int aim) {
        if (rest == 0) {
            return cur == aim ? 1 : 0;
        }
        // 走到这，代表还有剩余步数
    ...
    private static int process2(int cur, int rest, int N, int aim, int[][] dp) {
        // 有缓存取缓存
        if (dp[cur][rest] != -1) {
            return dp[cur][rest];
        }
        ...
        if (cur == 1) {
            ...
            ans = process1(cur - 1, rest - 1, N, aim) + process1(cur + 1, rest - 1, N, aim);
        }
        dp[cur][rest] = ans;
        return ans;
    }
    ...
    public static int robotWalk3(int N, int M, int K, int P) {
        ...
        /**
         *  if (rest == 0) {
         *             return cur == aim ? 1 : 0; // 这一步相当于确定0列
         *         }
         *         // 走到这，代表还有剩余步数
         *         if (cur == 1) {
         *             return process1(2, rest - 1, N, aim); // 只依赖左下的格子
         *         } else if (cur == N) {
         *             return process1(N - 1, rest - 1, N, aim);// 只依赖左上的格子
         *         } else {
         *              // 依赖左上与左下的格子，所以都是依赖左边的，所以大循环从左到右，小的只要从上到下就行
         *             return process1(cur - 1, rest - 1, N, aim) + process1(cur + 1, rest - 1, N, aim);
         *         }
         */
        // cur == aim ? 1 : 0,这一步相当于确定0列
        dp[P][0] = 1;
        for (int j = 1; j <= K; j++) {
            dp[1][j] = dp[2][j - 1];
            for (int i = 2; i < N; i++) {
